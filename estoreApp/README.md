...
- `estorewebApp/` - main app
  ...
  - `templates/`
  - `static/`
- `media/` - runtime upload path for local development; use object storage in production
...
This project is ready to deploy on Vercel with `estoreApp/` as the project root.

1. Push the repository to GitHub/GitLab/Bitbucket.
2. In Vercel, import the repository and set **Root Directory** to:
```text
estoreApp
```
3. Keep the build command from `vercel.json`:
```powershell
...
4. Add Vercel environment variables:
   ...
   - `PAYSTACK_PUBLIC_KEY`
   - `PAYSTACK_SECRET_KEY`
   - email variables if order/admin email should work
5. Deploy from Vercel.
6. Run migrations against the production database after deploy:
```powershell
vercel env pull .env.production.local
python manage.py migrate
```
Run that migration command from a trusted machine configured with the production `DATABASE_URL`.

Vercel notes:
- Vercel now detects Django from `requirements.txt`; `vercel.json` only customizes the static collection build step.
- The WSGI app exposes both `application` and `app` for Vercel Python runtime compatibility.
- User-uploaded files in `media/` are not persistent on Vercel serverless deployments. For production product/profile uploads, use persistent object storage such as Vercel Blob, S3, or Cloudinary.
- SQLite is not suitable for the hosted app. Use PostgreSQL via `DATABASE_URL`.

### 7.2 General Production Checklist
...
```
7. Configure the platform runtime. On Vercel this is handled by the Python runtime; on a VPS use a reverse proxy and WSGI server.
